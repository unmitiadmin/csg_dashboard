$(window).on("load", () => {
    ...
    let addOutcome = new AddOutcome(countryId, params);
    addOutcome.init();
});
...
class AddOutcome {
    constructor(countryId, params) {
        ...
        this.buttonCancelAdding = $("button#cancel_add_outcome");
        this.buttonSaveNewOutcome = $("button#save_new_outcome");
    }
    ...
    init = () => {
        ...
        .then(response => {
            if (response.data.role_id <= 3) this.execute();
                ...
                this.pageAlert("You are unauthorized to add an outcome", 0);
    ...
    submitForm = (reqBody) => {
        this.postApi(reqBody, this.authHeader, `projects/outcome?project_id=${this.projectId}`)
            .then(response => {
                if (response.success) {
            ...
            .catch(err => {
                ...
                if (err.status == 401){
                     this.pageAlert("Session expired, please login", 0);
                     setTimeout(() => window.location.replace("index.html"), 1000);
                } else this.pageAlert(err.responseJSON.message, 1);
            })
            .finally(() => this.stopWaiting())
    }
