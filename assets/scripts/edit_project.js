$(window).on("load", () => {
    // let countryId = parseInt(getCookies(document.cookie).countryId);
    // let userEmail = parseInt(getCookies(document.cookie).userId);
    // let editProject = new AddProject(countryId);
    let urlSearchParams = new URLSearchParams(window.location.search);
    const projectId = urlSearchParams.get('project_id')
    let editProject = new EditProject(projectId); // include init as a precheck for logged in user's role
    editProject.execute();
});
...
class EditProject{
    constructor(projectId){
        ...
        this.projectLabel = $("li#breadcrumb-project-name");
        ...
        this.textOrNumFields = [
            "project_name",
            "project_description",
            "funding_amount",
            "number_of_beneficiaries",
            "adaptation_description",
            // "adaptation_category_1",
            // "adaptation_category_2",
            // "adaptation_category_3",
            // "adaptation_category_4",
            // "adaptation_category_5",
            // "adaptation_category_6",
            // "adaptation_category_7",
            // "adaptation_category_8",
            // "adaptation_category_9",
            // "adaptation_category_10",
        ];
        ...
        this.buttonCancelEditing = $("button#cancel_edit_project");
        this.buttonSaveProject = $("button#save_edit_project");
    ...
    init = () => {
        ...
        .then(response => {
            if (response.data.role_id <= 2) this.execute();
                ...
                this.pageAlert("You are unauthorized to edit this project", 0);
    ...
    execute = () => {
        ...
        if(this.isLoggedIn){
            ...
            this.pageAlert("Please login", null);
            this.userCountryIcon.attr("src", null).attr("title", null).hide();
            setTimeout(() => window.location.replace("mande_tool.html"), 3000);
        }
    }
    ...
    getSelectOptionsFilled = () => {
        ...
        .then(response => {
            let [projectObj, lookups] = response;
            if(projectObj.success && lookups.success){
                let lookupData = lookups.data;
                ...
                let projectData = projectObj.data;
                this.projectData = projectData;
                this.projectLabel.empty().html(projectData["project_name"])
                ...
                Object.keys(this.lookupTableFields).forEach(a => {
                    ...
                    if(a == "province"){
                        ...
                        selectElement.selectpicker("val", projectData.province_id ? this.toStrArr(projectData.province_id) : []);
                        ...
                        selectElement?.selectpicker("destroy");
                        selectElement.selectpicker(this.selectpickerCommonOptions);
                        selectElement.selectpicker("val", projectData.district_id ? this.toStrArr(projectData.district_id) : []);
                        selectElement.selectpicker("render");
                    } else if(a == "country"){
                        selectElement.empty().html(this.lkpCountry.filter(b => b.id != 1).map(b => `<option value="${b.id}">${b[label]}</option>`).join("\n"));
                        selectElement?.selectpicker("destroy");
                        selectElement.selectpicker(this.selectpickerCommonOptions);
                        selectElement.selectpicker("val", projectData.multiple_country_ids ? this.toStrArr(projectData.multiple_country_ids) : [`${projectData.country_id}`]);
                        selectElement.selectpicker("render");

                        // if(projectData.multiple_country_ids) selectElement.trigger("change");
                        ...
                        selectElement?.selectpicker("destroy");
                        selectElement.selectpicker(this.selectpickerCommonOptions);
                        selectElement.selectpicker("val", selectElement.prop("multiple") ? this.toStrArr(projectData[a]) : projectData[a]);
                        selectElement.selectpicker("render");
                    } else if(a == "category"){
                ...
                // this.filterStartYear.selectpicker("val", projectData.start_year || "");
                // this.filterEndYear.selectpicker("val", projectData.end_year || "");

                this.filterStartYear.val(projectData.start_year || "");
                this.filterEndYear.val(projectData.end_year || "");
                // Text/Num inputs
                this.textOrNumFields.forEach(a => {
                    let inputElement = $(`input#input_${a}`);
                    let value = typeof(projectData[a]) == "string" ? projectData[a].replace(/�/g, "") : projectData[a]
                    inputElement.val(value);
                })
            } else this.pageAlert("Please check the api for lookups and project", 0);
        })
        .then(() => this.enableMultiProvince())
        .then(() => this.enableMultiCountryBudgetFields())
        .then(() => this.enableMultiTypology())
        .then(() => this.enableSubmit())
        .then(() => this.inputCountry.attr("disabled", !(this.userRoleId == 1)))
        .then(() => {
            if(this.projectData.funding_amount_by_country){
                let fundingAmountByCountry = this.projectData.funding_amount_by_country;
                fundingAmountByCountry.forEach(a => {
                    $(`input[type='number'].multicountry-budget-field[data-country-id='${a.country_id}']`).val(a.funding_amount)
                })
            }
        })
        .then(() => $("input[type='number'].multicountry-budget-field").trigger("input"))
        .then(() => {
            // this.inputCountry.attr("disabled", !(this.userRoleId == 1));
            this.inputCountry.trigger("change").selectpicker("refresh");
            
        })
        .then(() => {
            this.inputProvince.selectpicker("val", this.projectData.province_id).trigger("change");
            this.inputDistrict.selectpicker("val", this.projectData.district_id).trigger("change");
        })
        .catch(err => {
            console.error(err);
    ...
    enableMultiCountryBudgetFields = () => {
        this.inputCountry.unbind("change").on("change", () => {
            ...
            if (countryIds.length > 1) {
                // Check if any country fields should be removed
                ...
                // Remove all country fields when a single country is selected
                this.inputFundingAmount.val(this.projectData.funding_amount || "");
    ...
    prepareFormData = () => {
        this.requestBody = {
            // all form field values here, with respective names
            "country_id": this.inputCountry.val().length ==  1 
                    ? parseInt(this.inputCountry.val()[0] )
                    : 1 ,
    ...
    enableSubmit = () => {
        this.buttonSaveProject.unbind("click")
        .on("click", () => {
            this.prepareFormData()
            console.log(this.requestBody)
            setTimeout(() => this.submitFormData(this.requestBody), 1000);
        })
    }


    submitFormData = (requestBody) => {
        this.patchApi(requestBody, this.authHeader, `projects/manage?project_id=${this.projectId}`)
        .then(response => {
            if(response.success){
    ...
    toStrArr = (arr) => arr ? (arr.length ? arr.map(String) : []) : [];
