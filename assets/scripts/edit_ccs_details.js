$(window).on("load", () => {
    ...
    const dataId = urlSearchParams.get('data_id');
    let ccsFormEdit = new CCSFormEdit(dataId);
    ccsFormEdit.init();
});
...
class CCSFormEdit {
    constructor(dataId){
        ...
        this.subLocationFieldIds = [513, 556, 557, 514]; // If district id are present the subsequent data should be selected
        // already filled field data
        this.recordData = {};
        this.mediaPath = this.apiUrl == "http://localhost:8000" ? `csg_srilanka_api/media` : `csg_srilanka_media`;
    }
            ...
            if(a.type == "header"){
                ...
                if(a.is_group_child || a.type == "group"){
                    ...
                    return `<div class="row" data-field-row="${a.field_id}" data-tab-id="${a.tab_id}"
                        ...
                        <div class="col-sm-12 col-md-1 col-lg-2 field-center-filler" data-field-row="${a.field_id}"></div>
    ...
    collectFormData() {
        ...
        // use formDataObj to view in console.log()/debugging
    ...
    fillRecordData = () => {
        ...
        // Set ungroupedData
        Object.keys(ungroupedData).forEach(a => {
            let fieldElement = $(`[data-field-id="${a}"]`);
            let fieldTagName = fieldElement.prop("tagName").toLowerCase();
            let fieldElementType = fieldElement.prop("type");
            let value = ungroupedData[a];
            if (fieldTagName === 'input') {
                if (fieldElementType === 'radio' || fieldElementType === 'checkbox') {
                    $(`[data-field-id="${a}"][value="${value}"]`).prop("checked", true).trigger('click');
                } else if (fieldElementType === 'file') {
                    if (value) {
                        let fileIcon = this.getFileIcon(value);
                        let fileDisplay = `<a 
                            style="text-decoration: none !important; color: black;"
                            href="${this.apiUrl}/${this.mediaPath}/ccs_uploads/${value}"
                            title="${value}"
                            class="pull-right mt-3">
                            <i class="fa fa-2x ${fileIcon}"></i>
                        </a>`;
                        $(`div.field-center-filler[data-field-row="${a}"]`).empty().html(fileDisplay);
                    }
                } else {
                ...
                fieldElement.val(value).trigger('change');
                $(`select[data-type="lkp_district"]`).trigger("change");
            } else if (fieldTagName === 'textarea') {
                fieldElement.val(value).trigger('change');
            }
        });
        // Set groupedData
        groupedData.forEach(a => {
            let groupId = a.group_id;
            let groupIndex = a.group_index;
            let groupIndexData = a.group_index_data;
            Object.keys(groupIndexData).forEach(fieldId => {
                let value = groupIndexData[fieldId];
                let fieldElement = $(`[data-group-id="${groupId}"][data-group-index="${groupIndex}"][data-field-id="${fieldId}"]`);
                let fieldTagName = fieldElement.prop("tagName").toLowerCase();
                let fieldElementType = fieldElement.prop("type");
                if (fieldTagName === 'input') {
                    if (fieldElementType === 'radio' || fieldElementType === 'checkbox') {
                        $(`[data-group-id="${groupId}"][data-group-index="${groupIndex}"][data-field-id="${fieldId}"][value="${value}"]`)
                            .prop("checked", true)
                            .trigger('click');
                    } else {
                        fieldElement.val(value).trigger('change');
    ...
    getFileIcon = (fileName) => {
        ...
        switch (fileExtension) {
            ...
            case "doc": case "docx": return "fa-file-word-o text-primary";
            case "xls": case "xlsx": case "csv": case "tsv": return "fa-file-excel-o text-success";
            ...
            let newRowHtml = `<div class="row my-2 group-row" data-group-id="${groupId}" data-group-index="${currentIndex}">
                ...
                <div class="col-sm-1 col-md-1 col-lg-1">
                    <div class="btn-group">
                        ...
                        <!-- button type="button" class="btn btn-sm btn-success my-5 btn-add-group" data-group-id="${groupId}">
                            Add
                        </button -->
                    </div>
                </div>
        ...
        switch(true){
            ...
            case type == "lkp_district": // District -- idList == provinceIdList | API CALL FOR DS
                if(idList.length){
                    ...
                    $(`select[data-type="lkp_district"]`).empty().selectpicker(this.spOptions);
                }
                break;
            default:
                break;
    ...
    patchWithFile = (reqHead, reqBody, path) => {
        return new Promise((resolve, reject) => {
            $.ajax({
                "type": "PATCH",
                "url": `${this.apiUrl}/${path}?data_id=${this.dataId}`,
                "headers": reqHead,
                "data": reqBody,
                "processData": false,
                "contentType": false,
