window.addEventListener("load", (event) => {
    ...
    const headerContent = `<nav class="navbar navbar-expand-xl navbar-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarNav" style="flex-wrap: nowrap;">
            ...
            <ul class="navbar-nav ml-auto align-items-center">
                <li class="nav-item">
                    <div id="error-message" style="display: none; color: red; font-size: 12px; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">
    ...
    function googleTranslateElementInit() {
        ...
        try {
            if (window.google && window.google.translate && window.google.translate.TranslateElement) {
                ...
                updateOptions(); // Customize dropdown options with flags
                modifyTranslateWidget(); // Remove "Powered by"
            } else {
                scriptError = true;
            }
        } catch (error) {
    ...
    setInterval(() => {
        const selector = findGoogleTranslateSelector();
        if (selector?.type === "select" && selector.element.value && selector.element.value !== currentLanguage) {
            currentLanguage = selector.element.value;
            modifyTranslateWidget();
        }
    }, 2000);
})